  ...
  (
    ...
    setTimeout(() => {
      if (items.autoAuth.enabled) {
        fetch("https://www.pbinfo.ro/ajx-module/php-login.php", {
          method: "POST",
          headers: {
          ...
          body: `form_token=${
            document.querySelector(
              '#form-login-modal > div.modal-body > input[name="form_token"]'
            ).value
          }&local_ip=${document.getElementById("local_ip_header").value}&user=${
            autoAuth.username
          }&parola=${autoAuth.password}`,
        }).then((_) => {
    ...
    //       "meta>",
    //     ];
    //     if (excludedValues.includes(substring)) {
    //       return substring;
    //     } else {
    //       return "researcher_" + substring;
    //     }
    //   });
